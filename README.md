...
`avrdude` assumes that the `sck` programming parameter represents the count of `8000000/f` microseconds, where the `f` stands for the clock frequency of the target in Hz. However this firmware uses the same for the sck half period as the count of 1 microseconds.

`avrdude` uses the following formula to calculate this parameter (`dur`) from the entered value (`v`) in Terminal Mode.

```
dur = INT(v × 0.9216 + 0.5)
```
